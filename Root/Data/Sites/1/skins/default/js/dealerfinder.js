    ...
    if ($('#ddlProvince').length) {
        // $.ajax({
            ...
            // success: function(result) {
                ...
                // if (obj.find('option').length == 4) {
                    // obj.find('option').eq(3).prop('selected', true);
		...
		$.ajax({
            type: "GET",
            url: "/DealerLocator/Services/GeoZoneService.asmx/LoadCountry?languageId=-1" ,
            success: function(response) {
				var result = xml2json(response);
                var obj = $("#ddlCountry");
                $(obj).empty();
                $(obj).append($("<option></option>").val("").html($("#hdfDealerLocatorSelectCountry").val()));
                $.each(result.ArrayOfGeoZoneInfo.GeoZoneInfo, function(key, value) {
                    $(obj).append($("<option></option>").val(value.Key).html(value.Name));
					// console.log($(obj).html());
                });
        ...
        $('#ddlCountry').change(function() {
            $.ajax({
                type: "GET",
                url: "/DealerLocator/Services/GeoZoneService.asmx/LoadProvince?languageId=-1&countryGuid=" 
				 +$('#ddlCountry').val()
				,
        ...
        if ($('#ddlDistrict').length) {
            $('#ddlProvince').change(function() {
                $.ajax({
                    type: "GET",
                    url: "/DealerLocator/Services/GeoZoneService.asmx/LoadDistrict?languageId=-1&countryGuid=" + $('#ddlCountry').val() +"&provinceGuid="+ $('#ddlProvince').val(),
                    success: function(response) {
						var result = xml2json(response);
                        var obj = $("#ddlDistrict");
                        $(obj).empty();
                        ...
                        if (results.length > 0) {
                            ...
                            jQuery.each(results, function(k, v) {
                                ...
                                if (v.getAttribute("phone") != "") {
                                    phoneFax += "<p class='address'><span class='lnr lnr-phone-handset'>" + lang[currCulture].phone + ":</span> " + v.getAttribute("phone") + "</p>";
                                }
                                ...
                                if (number > 9) {
                                    $("<li id='l_" + marker['__gm_id'] + "' class='clinic_list double-digit' />").attr('data-map', v.getAttribute('des'))
                                        .html('<div class="icon"></div>' + '<address><p class="name">' + v.getAttribute('name') + '</p><p>' + v.getAttribute('address') + '</p><p>Tel: ' + v.getAttribute('phone') + '</p><p>Fax: ' + v.getAttribute('fax') + "</p><p class='hidden'><a class='hidden' href='" + v.getAttribute('website') + "'>" + v.getAttribute('website') + "</a></p></address>" + "<div class='link'>" + "<a class='view-map' href='#'>Xem bản đồ</a>" + "<a class='view-web' href='" + v.getAttribute('website') + "'>Website" + "</a></div>" + "<div id=d_" + number + " class='distance hidden'><span id='disval' attr-dist='" + distancenode + "' class='value'>" + distancenode + "</span> <span class='units'></span> <span class='time2'></span></div></div><div class='products'>" + ctype + "</div>")
                                        .click(function() {
